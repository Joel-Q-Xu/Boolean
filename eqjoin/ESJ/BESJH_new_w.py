...
"""for j in range(10):
    a = []
    for i in range(1):
        a.append([random.randint(0, 1) for _ in range(160)])
    time1 = time.time()
    x = null_space(a)
    time2 = time.time()
    print(time2 - time1)"""
...
def encryptQuery32(seedm, k, j_a,aaa,x_q,d,d_d):
...
def encryptQuery64(seedm, k, j_a,aaa,x_q,d,d_d):
...
def encryptQuery128(seedm, k, j_a,aaa,x_q,d,d_d):
...
def encryptQuery160(seedm, k, j_a,aaa,x_q,d,d_d):
