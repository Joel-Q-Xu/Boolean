...
def generateQueryVector(msk,k, x_q, l,size_a,size):
    (detB, B, Bstar, group, g1, g2) = msk

    # get coefficients of polynomial based on h_xq
    poly_coeffs=[0]*(size-2-size_a)
    for i in range(len(l)):
        h_xq = [group.hash(q) for q in x_q[i]]
        random_factor = group.random(ZR)
        # poly_coeffs = [0] * (x_q_max_size+1)
        h_p = [random_factor * coeff for coeff in generatePolynomial(h_xq)]

        poly_coeffs1 = (([0] * l[i]) + h_p + ([0] * (size - 2 - size_a - l[i]- len(h_p))))
        for i in range(len(poly_coeffs1)):
            poly_coeffs[i]+=poly_coeffs1[i]
    ...
    return [k]*size_a + poly_coeffs + [r, 0]
...
def encryptQuery(msk1,msk2, k, x_q,l,size_a,size ):
    return keygen(msk1,msk2, generateQueryVector(msk1, k, x_q,l,size_a,size))


def encryptTable(msk, table, pk, a, x, max_degree,aaa):
    #print([(row[pk], row[x], encryptRow(msk,a, row, max_degree,aaa)) for row in table])

    return [(row[pk], row[x[0]], encryptRow(msk,a, row, max_degree,aaa)) for row in table]
